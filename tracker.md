## Progress tracker

### Torch
**Basic**
- [X] [Custom Activation](torch/basic/custom-activation/custom-activation.ipynb)
- [X] [Custom Dataset](torch/basic/custom-dataset/custom-dataset.ipynb)
- [X] [Custom DNN](torch/basic/custom-DNN/custon-DNN.ipynb)
- [X] [Custom Loss](torch/basic/custom-loss/custom-loss.ipynb)
- [X] [Linear Regression](torch/basic/lin-regression/lin-regression.ipynb)
- [X] [Save Model](torch/basic/save-model/save_model.ipynb)
- [X] [tensorboard](torch/basic/tensorboard/tensorboard.ipynb)

**Easy**
- [X] [Augmentation](torch/easy/augmentation/augmentation.ipynb)
- [ ] [Autoencoder](torch/easy/autoencoder/autoencoder.ipynb)
- [ ] [Benchmark](torch/easy/benchmark/bench.ipynb)
- [ ] [CNN](torch/easy/cnn/CNN.ipynb)
- [ ] [CUDA-amp](torch/easy/cuda-amp/cuda-amp.ipynb)
- [ ] [Quantize LM](torch/easy/quantize-lm/quantize-language-model.ipynb)
- [ ] [RNN](torch/easy/rnn/RNN.ipynb)

**Medium**
- [ ] [3D CNN](torch/medium/3dcnn/3DCNN.ipynb)
- [ ] [AlexNet](torch/medium/alexnet/alexnet.ipynb)
- [ ] [CNN Param Init](torch/medium/cnn-param-init/CNN_ParamInit.ipynb)
- [ ] [CNN Scratch](torch/medium/cnn-scratch/CNN_scratch.ipynb)
- [ ] [LSTM](torch/medium/lstm/LSTM.ipynb)

### LLM